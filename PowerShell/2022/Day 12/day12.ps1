...
$nextsteps.Enqueue(@{X=$startx; Y=$starty})
...
while ($nextsteps.Count -gt 0) {
    ...
    #look left
    if ( ($x -gt 0) -and ($elevations[($x-1),$y] -le ($currentelevation + 1)) ) {
    ...
    #look right
    if ( ($x -lt $maxx-1) -and ($elevations[($x+1),$y] -le ($currentelevation + 1)) ) {
        if ((-not $visited[($x+1),$y]) -or ($steps[($x+1),$y] -gt $stepcount + 1)) {
            $visited[($x+1),$y] = $true
            $steps[($x+1),$y] = $stepcount + 1
            $nextsteps.Enqueue(@{X=($x+1); Y=$y})
    ...
    #look up
    if ( ($y -gt 0) -and ($elevations[$x,($y-1)] -le ($currentelevation + 1)) ) {
    ...
    #look down
    if ( ($y -lt $maxy-1) -and ($elevations[$x,($y+1)] -le ($currentelevation + 1)) ) {
        if ((-not $visited[$x,($y+1)]) -or ($steps[$x,($y+1)] -gt $stepcount + 1)) {
            $visited[$x,($y+1)] = $true
            $steps[$x,($y+1)] = $stepcount + 1
            $nextsteps.Enqueue(@{X=$x; Y=($y+1)})
...
$totalsteps = $steps[$endx,$endy]

Write-Host "Part 1: $totalsteps"